...
void setup() {
    ...
    myServo.write(90);  // Servo başlangıç pozisyonu 90 derece (orta)
...
void callback(char* topic, byte* payload, unsigned int length) {
    ...
    if (doc.containsKey("motor1")) {
        ...
        if (motor1Status == 1) {
            ...
            if (pressCount % 2 == 1) {
                ...
                analogWrite(ENB, motorSpeed); // Motor hızı ayarı
                motor2Active = true;
    ...
    // Servo motorun sağa dönme kontrolü (sağ buton)
    if (doc.containsKey("servoRight")) {
        int servoRight = doc["servoRight"].as<int>();

        if (servoRight == 1) {
            myServo.write(180); // Servo sağa dönsün (180 derece)
            Serial.println("Servo motor sağa (180 derece) döndü.");
        }
    }

    // Servo motorun sola dönme kontrolü (sol buton)
    if (doc.containsKey("servoLeft")) {
        int servoLeft = doc["servoLeft"].as<int>();

        if (servoLeft == 1) {
            myServo.write(0); // Servo sola dönsün (0 derece)
            Serial.println("Servo motor sola (0 derece) döndü.");
        }
    }
}
