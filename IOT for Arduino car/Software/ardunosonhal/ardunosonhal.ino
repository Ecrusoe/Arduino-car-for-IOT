...
bool motorOn = false; // Motorun açık olup olmadığını izlemek için
...
void setup() {
  ...
  analogWrite(motorPin, 0);  // Motor hızını başlangıçta sıfırla (kapalı)
...
void handleCommand(String command) {
  if (command == "MOTOR_ON") {
    motorOn = true;
    analogWrite(motorPin, motorSpeed); // Son ayarlanan hızla motoru aç
    Serial.println("Motor açıldı");
  } else if (command == "MOTOR_OFF") {
    motorOn = false;
    analogWrite(motorPin, 0); // Motoru kapat
    Serial.println("Motor kapatıldı");
  } else if (command == "MOTOR_INCREASE") {
    if (motorSpeed <= 235 && motorOn) {  // Motor hızı %100'ü aşmaması için kontrol
      motorSpeed += 25;  // Motor hızını %10 artır
      analogWrite(motorPin, motorSpeed);
      Serial.println("Motor hızı artırıldı");
    }
  } else if (command == "MOTOR_DECREASE") {
    if (motorSpeed >= 25 && motorOn) {  // Motor hızı %0'ın altına düşmemesi için kontrol
